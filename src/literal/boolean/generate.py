...
pub enum Boolean {""")
    ...
    print("")
    for line in lines:
        print(f"""fn parse_{line.lower()}(i: &[u8]) -> IResult<&[u8], Boolean> {{
    named!(semi, tag!(r#"{line}"#));
    map(semi, |_| Boolean::{snake_to_upper_camel(line)})(i)
}}""")
        print("")
    print(
        "pub fn parse(i: &[u8]) -> IResult<&[u8], Boolean> {")
    print("\talt((")
    for line in lines:
...
\t        ("a", Boolean::{snake_to_upper_camel(line)}))
\t}}""")
print("}")
